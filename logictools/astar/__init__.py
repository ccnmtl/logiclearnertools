...
class AStar:
    ...
    def heuristic_cost_estimate(self, current, goal):
        ...
        this method must be implemented in a subclass. The second parameter
        is always the goal."""
    ...
    def distance_between(self, n1, n2):
        """Gives the real distance between two adjacent nodes n1 and n2
           (i.e n2 belongs to the list of n1's neighbors). n2 is guaranteed to
           belong to the list returned by the call to neighbors(n1).
    ...
    def astar(self, start, goal, reversePath=False):
        ...
        while openSet:
            ...
            current.closed = True
            for neighbor in map(lambda n: searchNodes[n],
                                self.neighbors(current.data)):
                print("----next neighbor----")
                if neighbor.closed:
                    continue
...
def find_path(start, goal, neighbors_fnct, reversePath=False,
    ...
    class FindPath(AStar):
        ...
        def neighbors(self, node):
            ...
            if self.prev_considered is not None and node not in neighbors_fnct(
                                                        self.prev_considered):
                self.bailouts += 1
